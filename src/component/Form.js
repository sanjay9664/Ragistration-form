...
const Form = () => {
  ...
  {/* am changing the any name then give message you r ragistered successfully  */}
   <pre>{(fillform)?<h2 className='ui-define'>Hello {data.name} , You' r Ragistered  Successfull</h2>:""}</pre>
    ...
    <form className='container' onSubmit={handleSubmit}>  
      ...
      <div><input type="text"  name="name" value={data.name} onChange={handleClick}     placeholder='Enter your name' /></div>
      <div><input type="text"  name="email" value={data.email} onChange={handleClick}      placeholder='Enter your email' /></div>
      <div><input type="password"name="pass" value={data.pass} onChange={handleClick}     placeholder='Enter your password' /></div>
